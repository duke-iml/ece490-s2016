...
class PickingController:
    ...
    def scoopAction(self):
        ...
        if self.current_bin == None:
            ...
            if self.tilt_wrist('down', step = 1):
                ...
                self.move_camera_to_bin(self.current_bin, colMargin = 0, ik_constrain = False, ignoreColShelfSpatula=True, LOAD_IK=1)
                self.waitForMove()
    ...
    def unscoopAction(self):
        ...
        #     return False
            ...
            if self.move_camera_to_bin(self.current_bin, LOAD_IK = 1):
                ...
                self.move_camera_to_bin(self.current_bin, colMargin = 0,ik_constrain = False, LOAD_IK = 1)
                self.waitForMove()
    ...
    def move_spatula_to_center(self, ik_constrain=True):
        ...
        for i in range(100):
            ...
            if path == 1 or path == 2 or path == False:
                ...
                if SAVE_MOVE_SPATULA_TO_CENTER_TRAJECTORY:
                    saveToFile(path, "Trajectories/"+self.current_bin+"_spatula_to_center")
                if SKIP_PATH_VIEWING:
                    self.controller.setMilestone(solution[0])
                    return True
                if ik_constraint==None:
                    self.sendPath(path)
                else:
    ...
    def move_gripper_to_center(self):
        ...
        for i in range(100):
            if not SAVE_MOVE_GRIPPER_TO_CENTER_IK:
            ...
            if len(sortedSolutions)==0:
                ...
                for solution in sortedSolutions:
                    ...
                    if path == 1 or path == 2 or path == False:
                        ...
                        if SAVE_MOVE_GRIPPER_TO_CENTER_IK:
                            saveToFile(solution, "IK_Solutions/"+"gripper_to_center")
                        if SAVE_MOVE_GRIPPER_TO_CENTER_TRAJECTORY:
                            saveToFile(path, "Trajectories/"+"gripper_to_center")
                        if SKIP_PATH_VIEWING:
                            self.controller.setMilestone(sortedSolutions[0][0])
                            return True
                        else:
    ...
    def placeInOrderBinAction(self):
        ...
        if self.move_gripper_to_center():
            ...
            print "Successfully placed object into order bin"
    ...
    def move_camera_to_bin(self,bin_name, colMargin = 0.05, ik_constrain=True,ignoreColShelfSpatula=True,LOAD_IK=0):
        self.robot.setConfig(self.controller.getCommandedConfig())
        ...
        for i in range(100):
            ...
            if LOAD_IK or not SAVE_MOVE_TO_BIN_IK:
                print "loading the IK soluton to view bin"
            ...
            if len(sortedSolutions)==0:
                ...
                for solution in sortedSolutions:
                    ...
                    if path == 1 or path == 2 or path == False:
                        ...
                        if step==2 and SAVE_MOVE_GRIPPER_TO_ORDER_BIN_TRAJECTORY:
                            saveToFile(path, "Trajectories/"+"move_to_order_bin")
                        if SKIP_PATH_VIEWING:
                            self.controller.setMilestone(sortedSolutions[0][0])
                            return True
                        else:

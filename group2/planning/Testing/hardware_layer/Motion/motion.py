"""Python interface to almost all of the APC motion functionality
(excluding sensors)
...
Can switch between simulation and real robot using the 'mode' keyword
argument to setup().
...
Caveats:
 - Mobile base is not working yet.
...
class EndEffector:
    ...
    def moveTo(self,R,t):
        ...
        return motion_lib.sendEndEffectorMoveTo(self.limb,self.tempr_p,self.tempp_p)
...
class Gripper:
    ...
    def _init_buffers(self):
        if self.temp == None:
            self.temp = (c_double*self.numDofs())()
            self.tempv = (c_double*self.numDofs())()
            self.tempf = (c_double*self.numDofs())()
            self.temp_p = cast(self.temp,POINTER(c_double))
...
class LimbMotionQueue:
    ...
    def setTrajectory(self,times,milestones,dmilestones=None):
        ...
        if dmilestones != NULL:
            ...
            return motion_lib.sendMotionQueueTrajectory(self.limb,k,cast(ttemp,POINTER(c_double)),cast(mtemp,POINTER(c_double)))
    def appendTrajectory(self,times,milestones,dmilestones=None):
        ...
        if dmilestones != NULL:
            ...
            return motion_lib.sendMotionQueueAppendTrajectory(self.limb,k,cast(ttemp,POINTER(c_double)),cast(mtemp,POINTER(c_double)))
...
def setup(mode=None,libpath="hardware_layer/",klampt_model=None):
    ...
    Arguments:
    - mode: can be None, 'kinematic', or 'physical', depending on whether
      you want to simulate or operate the real robot.  If it's none, then it
      will use whatever is currently linked to by libmotion.so
    ...
    if klampt_model != None:
        robot.setKlamptModel(klampt_model)
    print
...
if __name__=="__main__":
    ...
    if len(sys.argv)>1:
        mode = sys.argv[1]
    robot = setup(mode=mode,klampt_model=klampt_model)
    res = robot.startup()
    if not res:
        raise RuntimeError("APC Motion could not be started")

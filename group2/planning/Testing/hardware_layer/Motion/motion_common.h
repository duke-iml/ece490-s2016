void* controller_thread_func(void* vdata)
{
  ...
  if(!data->systemStateAddr.empty()) {
    printf("APC Motion: Connecting to system state server...\n");
    if(!data->systemStateService.Open(data->systemStateAddr.c_str(),FILECLIENT)) {
      FatalError("APC Motion: Could not open socket to system state service %s",data->systemStateAddr.c_str());
    }
  }
  {
  ...
  //RUNNING.....!
  // printf("APC Motion: Running low-level control loop\n");
  Timer mytimer;
...
{
  if(gData.robotModel) {
    printf("setKlamptModel(): Warning, robot model was already set before\n");
  }
  gData.robotModel = new Robot;
  if(!gData.robotModel->Load(klampt_model)) return 0;
  gData.OnWorldChange();
  return 1;
}
...
void* getKlamptModel()
{
  if(gData.robotModel==0) return NULL;
  return gData.robotModel;
}
...
BOOL sendStartup()
{
  if(gData.startup || gData.running ) {
    printf("APC Motion: sendStartup(): Error, APC Motion already running\n");
    return 0;
  }
  gData.kill = false;
  gData.failure = false;
...
BOOL sendHeadPan(double target,double speed)
{
  ...
  if(gData.robotState.head.sendCommand) {
    printf("APC Motion: Warning, last sendHeadPan command wasn't processed\n");
  }
...
APIENTRY BOOL getEndEffectorSensedTransform(int limb,double* rotation,double* position)
{
  ...
  gData.robotModel->links[index].T_World.t.get(position);
  return 1;
}
APIENTRY BOOL getEndEffectorSensedVelocity(int limb,double* angVel,double* vel)
{
  ...
  gData.robotModel->UpdateSelectedFrames(index);
  Vector3 dp,dw;
  gData.robotModel->GetWorldVelocity(Vector3(0.0),index,gData.robotModel->dq,dp);
...
{
  //TODO: IK
  printf("sendEndEffectorMoveTo not implemented yet\n");
  return 0;
}
...
APIENTRY BOOL setKlamptLimb(const double* limbConfig,int limb,double* klamptConfig)
{
  ...
  if(limb == LEFT || limb == RIGHT) {
    Vector q(gData.robotModel->q.n,klamptConfig),qlimb(numLimbDofs);
    Copy(limbConfig,qlimb);
    gData.LimbToKlampt(qlimb,limb,q);
    Copy(q,klamptConfig);
  }
  else {
    Vector q(gData.robotModel->q.n,klamptConfig),qlimb(numLimbDofs*2);
    Copy(limbConfig,qlimb);
    gData.LimbToKlampt(qlimb,limb,q);
    Copy(q,klamptConfig);
  }

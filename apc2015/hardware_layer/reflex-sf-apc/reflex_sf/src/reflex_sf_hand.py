...
class ReflexSFHand(object):
    ...
    def receiveCmdCb(self, data):
        if len(data.position)==0 and len(data.force)==0 and len(data.velocity)==0:
            print "reflex_sf_hand.py: Warning, received empty command"
        print "Command:",data.position
        print "Force:",data.force
        print "Speed:",data.velocity
        ...
        if len(data.force) > 0:
            self.motors[self.prefix+'_f1'].setForceThreshold(data.force[0])
            self.motors[self.prefix+'_f2'].setForceThreshold(data.force[1])
            self.motors[self.prefix+'_f3'].setForceThreshold(data.force[2])
            self.motors[self.prefix+'_preshape'].setForceThreshold(data.force[3])
        if len(data.velocity) > 0:
            self.motors[self.prefix+'_f1'].setSpeed(data.velocity[0])
            self.motors[self.prefix+'_f2'].setSpeed(data.velocity[1])
            self.motors[self.prefix+'_f3'].setSpeed(data.velocity[2])
            self.motors[self.prefix+'_preshape'].setSpeed(data.velocity[3])            
    ...
    def motorReceiveCb(self,name,data):
        ...
        self.state.current_pos[index] = motor.current_commanded_position
        self.state.error[index] = data.error * motor.raw_direction
        self.state.velocity[index] = data.velocity * motor.raw_direction
        self.state.load[index] = data.load * motor.raw_direction
        if motor.is_moving: self.state.is_moving = True
    ...
    def calibrate(self):
        for motor in self.motors.itervalues():
            motor.enableController(False)
        for motor in sorted(self.motors):
            
            rospy.loginfo("Calibrating motor " + motor)
            print  "Current raw position:",self.motors[motor].current_raw_position
            command = raw_input("Type 't' to tighten motor, 'l' to loosen \
motor, 'q' to indicate that the zero point has been reached \n\
or 'a' to abort.\n")
            while command.lower() not in ['q','a']:
                if command.lower() == 't':
                    ...
                    print "Didn't recognize that command, use 't', 'l', 'q', or 'a'"
                print  "Current raw position:",self.motors[motor].current_raw_position
                command = raw_input("Tighten: 't'\tLoosen: 'l'\tDone: 'q'\n")
            if command.lower() == 'a':
                print "Aborting calibration..."
                for motor in self.motors.itervalues():
                    motor.enableController(True)
                return

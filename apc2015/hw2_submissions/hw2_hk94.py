...
class KnowledgeBase:
    ...
    def __init__(self):
        ...
        self.shelf_xform = se3.identity()    # <-- assume this is estimated perfectly for this hw.
    ...
    def bin_front_center(self,bin_name):
        ...
        #TODO:
        # MyGLViewer.draw_bins = True
#         print "**********************just ran bin_front_center"
        corner1,corner2 = apc.bin_bounds[bin_name]
        centerX, centerY, centerZ = [(corner1[0] + corner2[0])/2, (corner1[1] + corner2[1])/2, corner2[2]]

        return se3.apply(ground_truth_shelf_xform, [centerX, centerY, centerZ])
        # return None
    ...
    def bin_vantage_point(self,bin_name):
        ...
        #'draw_bins' is turned to True.
        #TODO:
#         print "**********************just ran bin_vantage_point"
        x,y,z = self.bin_front_center(bin_name)
        return [x-0.2,y,z]
        #return se3.apply(ground_truth_shelf_xform, [(x/2 for x in apc.bin_bounds[bin_name][1])])
        #return ((se3.apply(ground_truth_shelf_xform, apc.bin_bounds[bin_name][0])))


        #return apc.bin_bounds[bin_name][1]  # <--- needs to be transformed right
        # print ground_truth_shelf_xform # <--- maybe use this for transform
...
class PickingController:
    ...
    - config: the robot's current configuration
    ...
    def __init__(self,world):
        ...
        self.startingPos = self.robot.getConfig()
    ...
    def move_camera_to_bin(self,bin_name):
        ...
        # potentially useful... transform (R,t) to world frame
        # print self.left_camera_link.getTransform()
        # print self.left_camera_link.getLocalPosition([0, 0, 0])
        # print self.left_camera_link.getWorldPosition([0, 0, 0])

#         print "vantage: ", worldVantagePoint
#         goal = ik.objective(self.left_camera_link,local = [0,0,0], world = worldVantagePoint)
#         if ik.solve(goal):
#             print "ik solved"
#             self.config = self.robot.getConfig()


#         currLoc = self.left_camera_link.getWorldPosition([0,0,0])
#         print "world position: ", currLoc

#         xform = self.knowledge.shelf_xform
#         goal = ik.objective(self.left_camera_link, R=xform[0], t=[0,0,0])

#         print "numLinks", self.robot.numLinks()
#         print "joint limits min : ", self.robot.getJointLimits()[0]
#         print "joint limits max : ", self.robot.getJointLimits()[1]
#         print "config: ", self.robot.getConfig()

        worldVantagePoint = self.knowledge.bin_vantage_point(bin_name)
        curr = self.robot.getConfig()
        randinit = curr[:]
        minLimit = self.robot.getJointLimits()[0]
        maxLimit = self.robot.getJointLimits()[1]

        counter = 1
        while (True):
            print counter,
            counter += 1
            for i in range(0, self.robot.numLinks()):
                randinit[i] = random.uniform(minLimit[i], maxLimit[i])
            self.robot.setConfig(randinit)

            for i in range(0,2):
                if i==0:
                    self.active_limb = 'left_limb'
                    XtoZ = so3.from_axis_angle(([0,1,0],math.pi*0.5))
                    XtoY = so3.from_axis_angle(([0,0,1],math.pi*0.5))
                    xform = self.left_camera_link.getTransform()
                    goal = ik.objective(self.left_camera_link, R=so3.mul(XtoZ, XtoY), t=worldVantagePoint)

                    if ik.solve(goal):
                        print "ik solved"
                        self.config = self.robot.getConfig()[0:27] + self.startingPos[27:54]
                        # self.config = self.robot.getConfig()
                        print self.config
                        return True

                if i==1:
                    self.active_limb = 'right_limb'
                    XtoZ = so3.from_axis_angle(([0,1,0],math.pi*0.5))
                    XtoY = so3.from_axis_angle(([0,0,1],math.pi*0.5))
                    xform = self.right_camera_link.getTransform()
                    goal = ik.objective(self.right_camera_link, R=so3.mul(XtoZ, XtoY), t=worldVantagePoint)

                    if ik.solve(goal):
                        print "ik solved"
                        self.config = self.startingPos[0:27] + self.robot.getConfig()[27:54]
                        # self.config = self.robot.getConfig()
                        print self.config
                        return True
    ...
    def display(self):
        ...
        if self.draw_bins:
            ...
            for b in apc.bin_bounds.values():
                # displays the boundaries of the bin in yellow
                # print "skip drawing bin boundaries in yellow"
        ...
        if self.draw_gripper_and_camera:
            ...
            gldraw.xform_widget(se3.mul(left_camera_link.getTransform(),left_gripper_center_xform),0.05,0.005)
            gldraw.xform_widget(se3.mul(right_camera_link.getTransform(),right_gripper_center_xform),0.05,0.005)

...
left_arm_link_names = ['left_upper_shoulder','left_lower_shoulder','left_upper_elbow','left_lower_elbow','left_upper_forearm','left_lower_forearm',
'left_wrist']
right_arm_link_names = ['right_upper_shoulder','right_lower_shoulder','right_upper_elbow','right_lower_elbow','right_upper_forearm','right_lower_forearm',
'right_wrist']
...
class KnowledgeBase:
    ...
    def bin_front_center(self,bin_name):
        ...
        #TODO:
		bounds = apc.bin_bounds[bin_name]
		point = [(bounds[0][i] + bounds[1][i])/2 for i in range(3)]
		boxDepth = abs(bounds[0][2] - bounds[1][2])
		point[2] += boxDepth/2.0 + .2
    ...
    def grasp_xforms(self,object):
        ...
        #TODO:
		#kaighn modified
		grasps_list = object.info.grasps
		grasps_list = grasps_list
		return [se3.mul(object.xform,grasp.grasp_xform) for grasp in grasps_list]
...
class PickingController:
	...
	self.originalConfig = self.robot.getConfig()
    ...
    def fulfillOrderAction(self,objectList):
        ...
        #sensing
	visitedDict = {}
	bins_left = [x for x in apc.bin_names]
	for bin_name in apc.bin_names:
		visitedDict[bin_name] = False
	while(len(bins_left) != 0):
		for bin_name in bins_left:
			print bin_name
			if self.viewBinAction(bin_name):
				visitedDict[bin_name] = True
				bins_left.remove(bin_name)
	
	objectCount = {}
	for bin_name in apc.bin_names:
		for item in self.knowledge.bin_contents[bin_name]:
			print "item",item
			print "item name?", item.info.name
			objectCount[item.info.name] = 0
	for bin_name in apc.bin_names:
		for item in self.knowledge.bin_contents[bin_name]:
			objectCount[item.info.name] = objectCount[item.info.name] + 1
	
	orderCount = {}
	for item in objectList:
		orderCount[item] = 0
	for item in objectList:
		orderCount[item] = orderCount[item] + 1
	...
	for item in objectList:
		...
		b = objectCount[item]
		c = a + b
		if orderCount[item] < objectCount[item]:
			print 'Object requested either does not exist or theres not enough of it'
	
	print "DONE SENSINGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG"
	#placing objects
	numberOfDesiredItems = len(objectList)
	numberOfFulfilledItems = 0
	print len(objectList)
	iterations = 0
	while (iterations != 1000) and (numberOfFulfilledItems < numberOfDesiredItems):
		print 'while loop reached'
		iterations+=1
		for item in objectList:
			print "objectList", objectList
			for bin_name in apc.bin_names:
				itemNameInBin = [thing.info.name for thing in self.knowledge.bin_contents[bin_name]]
				print "items in bin", bin_name, ":  ", itemNameInBin
				print "itemYo : ", item, "itemNameInBin: ", itemNameInBin, item in itemNameInBin
				if item in itemNameInBin:
					print item, "in bin", bin_name
					if self.viewBinAction(bin_name):
						print "viewed successfully"
						if self.graspAction():
							print "grasped successfully"
							if self.placeInOrderBinAction():
								print "placed into order bin successfully"
								numberOfFulfilledItems += 1
	print numberOfDesiredItems
	print numberOfFulfilledItems
	if numberOfDesiredItems != numberOfFulfilledItems:
		print "FAIL"
		return False
	...
	if random.random() < .5:
		goal = ik.objective(self.left_camera_link,R=[0,1,0,0,0,1,1,0,0],t=pointInFrontOfBin)
		self.active_limb = self.left_camera_link
	else:
		self.active_limb = self.right_camera_link
		goal = ik.objective(self.right_camera_link,R=[0,1,0,0,0,1,1,0,0],t=pointInFrontOfBin)
	iterations = 0
	correctBin = True
	
	while iterations != 1000:
		if ik.solve(goal) & correctBin:
			self.config = self.robot.getConfig()
			return True
		else:
			self.config = self.robot.getConfig()
			if random.random() < .3:
				correctBin = False
				randomBinName = random.choice(apc.bin_names)
				randomPointInFrontOfBin = self.knowledge.bin_front_center(randomBinName)
				if random.random() < .5:
					goal = ik.objective(self.left_camera_link,R=[0,1,0,0,0,1,1,0,0],t=randomPointInFrontOfBin)
					self.active_limb = self.left_camera_link
				else:
					self.active_limb = self.right_camera_link
					goal = ik.objective(self.right_camera_link,R=[0,1,0,0,0,1,1,0,0],t=randomPointInFrontOfBin)
			else:
				correctBin = True
				if random.random() < .5:
					self.active_limb = self.right_camera_link
	...
	while iterations != 1000:
		if ik.solve(goal) & correctBin:
			...
			if iterations%2==0:
				correctBin = False
				nearPoint = [pointInFrontOfBin[i] + random.random()*.2-.1 for i in range(3)]
				if random.random() < .5:
					self.active_limb = self.right_camera_link
					goal = ik.objective(self.right_camera_link,R=[0,1,0,0,0,1,1,0,0],t=nearPoint)

				else:
					self.active_limb = self.left_camera_link
					goal = ik.objective(self.left_camera_link,R=[0,1,0,0,0,1,1,0,0],t=nearPoint)
...
class MyGLViewer(GLNavigationProgram):
    ...
    def __init__(self,world):
        ...
        self.draw_bins = False #Kaighn modified

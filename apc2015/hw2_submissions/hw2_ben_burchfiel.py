...
class KnowledgeBase:
    ...
    def bin_vantage_point(self,bin_name):
        boundingBox = apc.bin_bounds[bin_name]
        binFrontCenterShelfFrame = [(boundingBox[0][0] + boundingBox[1][0]) / 2, (boundingBox[0][1] + boundingBox[1][1]) / 2, boundingBox[1][2] ]
        frontCenter = self.bin_front_center(bin_name)
        frontCenter[0] = frontCenter[0] - 0.1
        return frontCenter
...
class PickingController:
    ...
    def fulfillOrderAction(self,objectList):
        ...
        # this function assumes only 1 object per bin
        bins = ['A','B','C','D','E','F','G','H','I','J','K','L']
        for bin in bins:
            if not self.viewBinAction('bin_'+bin):
                return False
            objInBin = self.knowledge.bin_contents['bin_'+bin]
            for obj in objectList: # see if this bin has a matching object
                if objInBin and obj == objInBin[0].info.name: # fill order
                    objToMove = objInBin[0]
        ...
        return not objectList # if we filled all list orders, return true
    ...
    def move_camera_to_bin(self,bin_name):
        ...
        p1 = self.knowledge.bin_vantage_point(bin_name)
        p2 = [p1[0] + .1, p1[1], p1[2]]
        worked = False

        # try left arm
        oldConfig =  self.robot.getConfig()
        goal = ik.objective(self.left_camera_link, local=[[0, 0, 0]], world=[p1])
        ik.solve(goal)
        goal = ik.objective(self.left_camera_link, local=[[0, 0, 0], [0, 0, .1]], world=[p1, p2])
        if ik.solve(goal):
            worked = True
            ...
            goal = ik.objective(self.right_camera_link, local=[[0, 0, 0]], world=[p1])
            ik.solve(goal)
            goal = ik.objective(self.right_camera_link, local=[[0, 0, 0], [0, 0, .1]], world=[p1, p2])
            if ik.solve(goal):
                worked = True
                self.active_limb = "Right"
            else:
                self.robot.setConfig(oldConfig)
                print "IK solver failed to find camera to bin movement."
                return False

...
def get_closest_points_2d(P1, P2):
    ...
    for p1 in P1:
        best = P2[0]
        best_dist = euclidean_distance_2(p1,P2[0])
        for p2 in P2[1:]:
            d = euclidean_distance_2(p1,p2)
            if d < best_dist:
                best_dist = d
                best = p2
        P2match.append(best)
...
def threshold_closest_points(P1, P2, threshold):
    ...
    for p1,p2 in zip(P1,P2):
        if euclidean_distance_2(p1,p2) <= threshold:
            P1_th.append(p1)
            P2_th.append(p2)
    return P1_th, P2_th
...
def get_correspondences(P1, P2):
    ...
    threshold = 0
    frac = 0.1
    #TODO: determine a good threshold
    d = sorted(euclidean_distance_2(p1,p2) for (p1,p2) in zip(P1,P2matches))
    threshold = d[int(frac*len(d))]
...
def subtract_mean(point_list):
    ...
    """
    mean_x = float(sum(p[0] for p in point_list))/len(point_list)
    mean_y = float(sum(p[1] for p in point_list))/len(point_list)
    point_list_minus_mean = [[p[0]-mean_x,p[1]-mean_y] for p in point_list]
...
def compute_error_minimizing_rotation(Points1, Points2):
    ...
    sol1 = numpy.dot(U,V.T)
    sol2 = numpy.dot(V,U.T)
    if sum_of_squared_errors(Points1,transform_points(Points2,sol1,[0,0])) < sum_of_squared_errors(Points1,transform_points(Points2,sol2,[0,0])):
        return sol1
    else:
        return sol2
...
file_triangle1 = "data/2d/image_1.tif"
file_triangle2 = "data/2d/image_2.tif"

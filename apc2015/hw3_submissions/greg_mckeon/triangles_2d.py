...
def get_closest_points_2d(P1, P2):
    ...
    for point in range(len(P1)):
        ...
        for point2 in range(len(P2)):
            if euclidean_distance_2(P1[point],P2[point2])<myMin:
                myMin = euclidean_distance_2(P1[point],P2[point2])
                smallest = P2[point2]
        out.append(smallest)
    return out
...
def threshold_closest_points(P1, P2, threshold):
    ...
    newP2 = []
    for point in range(len(P1)):
        if euclidean_distance_2(P1[point],P2[point])<threshold:
...
def get_correspondences(P1, P2):
    ...
    for i in range(len(P1)):
        listy.append(euclidean_distance_2(P1[i], P2matches[i]))
    sort = sorted(listy)
    threshold = sort[(int)(len(P1)*.9)]
    ...
    # in the below:
    #
    # I took 90% as the threshold for this specific assignment because almost all the points were "useful" - there was no real noise/outliers in the image, and I 
    # mainly needed to avoid the case where multiple points might map to the same point on the target.  This is a high threshold - much higher than might be used for the 
    # 3d case, and much higher than would be used if more obstacles/noice existed in the image.  This number was further refined through trial and error to give low error.
    #
...
def subtract_mean(point_list):
    ...
    for j in range(len(point_list)):
        point_list_minus_mean.append([point_list[j][0]-mean_x,point_list[j][1]-mean_y])
...
def compute_error_minimizing_rotation(Points1, Points2):
    ...
    U, s, V = numpy.linalg.svd([[spot11, spot12],[spot21,spot22]])
    M = numpy.transpose(V)
    return numpy.dot(numpy.transpose(U), V)

...
def get_closest_points_2d(P1, P2):
    ...
    for i in range(len(P1)):
    	...
    	for j in range(len(P2)):
    		if euclidean_distance_2(P1[i],P2[j]) < bestDist:
    			bestDist = euclidean_distance_2(P1[i],P2[j])
    			bestPoint = P2[j]
...
def get_correspondences(P1, P2):
    ...
    test = [euclidean_distance_2(P1[i],P2matches[i]) for i in range(len(P1))]
    sort = sorted(test)
    threshold = sort[(int)(len(P1)*.8)]
...
def subtract_mean(point_list):
    ...
    point_list_minus_mean = [[point_list[i][0]-mean_x,point_list[i][1]-mean_y] for i in range(len(point_list))]
...
def compute_error_minimizing_rotation(Points1, Points2):
    ...
    tr = sum([Points1[i][0] * Points2[i][1] for i in range(len(Points1))])
    bl = sum([Points1[i][1] * Points2[i][0] for i in range(len(Points1))])
    br = sum([Points1[i][1] * Points2[i][1] for i in range(len(Points1))])
    cov = [[tl,tr],[bl,br]]

...
order_bin_xform = (so3.identity(),[0.75,0,0])
...
class PickingController:
    ...
    def graspDemo(self):
        # Assume that order bin is a box which will have object on it
        # assume we have object's transform

        objectPath = "../klampt_models/items/feline_greenies_dental_treats/grasps/reflex/0.json"

        # read JSON file with object grasps
        jsonstr = open(objectPath, "r").read()
        graspJson = json.loads(jsonstr)
        objectTransform = graspJson["grasp_xform"]
        openConfig = graspJson["gripper_open_command"]
        closedConfig = graspJson["gripper_close_command"]

        # go through list, find ik solution
        # we'll do this latter... for now assume we have one grasp that will work

        # set hand config to open grasp
        world.robot(0).setConfig(


        # use ik solution to plan path


        # execute path
        # set hand config to close grasp

        # do silly demo stuff (wave object arround or something)
...
def ontroller(controller,command_queue):

...
withdraw_point_xforms = json.load(open('kb/withdraw_point_xforms.json'))
...
class Viewer(WorldViewer):
    def display_screen(self):
        ...
        labels = dict([ (k, T[1]) for (k, T) in withdraw_point_xforms.items() ])
...
viewer = Viewer(world, withdraw_point_xforms.values())
viewer.run()

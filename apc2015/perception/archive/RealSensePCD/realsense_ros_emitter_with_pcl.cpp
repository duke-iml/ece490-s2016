  ...
  */
  ...
  for(int i=0;i<pc_msg.width;i++) {
    for(int j=0;j<pc_msg.height;j++,k++) {
      ...
      float *x = (float *)indata;
      float *y = (float *)indata+1;
      float *z = (float *)indata+2;
      char b=indata[12];
      char g=indata[13];
      char r=indata[14];
      ...
      *(float*)(outdata+20) = float(b)/255.0;
      ...
      unsigned char ri = (unsigned char)r;
      unsigned char bi = (unsigned char)b;
      unsigned char gi = (unsigned char)g;
      uint32_t rgb = (static_cast<uint32_t>(ri) << 16 | static_cast<uint32_t>(gi) << 8 | static_cast<uint32_t>(bi));
      float rgbf = *reinterpret_cast<float*>(&rgb);
      if(isnan(rgbf)) {
        cout << (int)r << " " << (int)g << " " << (int)b << "\n";
        cout << static_cast<uint32_t>(ri) << "\n";
      }
      if(*x==0 && *y==0 && *z==0) {
        continue;
      }
  ...
  pcl::io::savePCDFile<pcl::PointXYZRGB> ("test_pcd2.pcd", cloud);
  f.close();
  exit(0);
  if(visualize) {

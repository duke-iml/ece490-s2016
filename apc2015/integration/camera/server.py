...
def parse_args():
    ...
    return (device_index, address, port)
...
def setup_camera(device_index):
    # enumerate all devices
    try:
        devices = list(Camera.list(StreamType.COLOR | StreamType.DEPTH | StreamType.IR))
    except:
    ...
    try:
        ...
        logger.error('invalid device index: {}'.format(device_index))
        raise SystemExit
...
if __name__ == '__main__':
    try:
        device_index, address, port = parse_args()
    except Exception:
        print sys.argv[0], '[device index]', '[bind address[:port]]'
        raise SystemExit

    sock = setup_socket(address, port)

    camera = setup_camera(device_index)
    app = QApplication(sys.argv)

    logger.info('waiting for clients')

    try:
        loop(sock, camera, app, show=False)
    finally:
        try:
            camera.close()
        except Exception as e:

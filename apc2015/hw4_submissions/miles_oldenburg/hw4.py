...
def run_perception_on_bin(knowledge, bin_name, planner=None):
    ...
    if knowledge.bin_contents[bin_name]==None:
        ...
        for item in ground_truth_items:
            if item.bin_name == bin_name:
                ...
                # Add item to planner world and reinit collider
                make_object_in_world(planner.world, item)
                planner.reinit_collider()
    return
...
class PickingController:
        ...
        if self.state != 'ready':
            ...
            if b in apc.bin_names:
                if self.move_camera_to_bin(b):
                    self.waitForMove()
                    self.current_bin = b
                    run_perception_on_bin(self.knowledge, b, planner=self.planner)
    ...
    def fulfillOrderAction(self,objectList):
        ...
        for b in apc.bin_names:
            ...
            if len(remainingObjects)==0:
                print "solutions", self.planner.solutions, "collisions", self.planner.collisions
                return True
    ...
    def move_to_grasp_object(self,object):
        ...
        for (grasp,gxform) in grasps:
            ...
            self.robot.setConfig(qcmd)
            if ik.solve(goal):
                if self.planner.check_collision_free(self.active_limb):
                    self.controller.setMilestone(self.robot.getConfig())
...
def make_object_in_world(world, item):
...
def spawn_objects_from_ground_truth(world):
    ...
    for item in ground_truth_items:
        make_object_in_world(world, item)
    return
...
# Q1.
# 1. The arms are allowed to move through the shelf when moving towards a specific bin. Collision detection should be enabled on the shelf.
# 2. The arms are allowed to move through the Baxter body when moving towards a specific bin. Collision detection should be enabled on the Baxter body.
# 3. When aligning to a grasp on the object, the robot arm can find a solution that would cause the arm to collide with the shelf. Potential grasps should return only those which do not cause collision with the shelf.
# 4. After an object has been grasped and instruction given to put the object into the bin, the motion can collide with the shelf. Collision detection and motion planning should take into account the shelf.
#
# Q2.
# I use the provided WorldCollider to check for possible self collisions.
#
# If there was a collision, then the solved config is not set and enters the loop again where it is randomized and another solution attempted. If there was no collision, the config is set and the motion is started.
#
# I ran 5 trials using the full order bin action and recorded the number of solutions and collisions. In my trials on average ~56% of IK solutions were in self-collision.
#
# Trial 1: 10/24
# Trial 2: 25/39
# Trial 3: 24/38

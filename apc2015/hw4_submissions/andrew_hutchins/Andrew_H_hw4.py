...
import sys
sys.path.append(os.path.join(os.path.expanduser("~"),"ece590-s2015","apc"))
sys.path.append(os.path.join(os.path.expanduser("~"),"ece590-s2015","klampt_models"))
print sys.path
from baxter import *
        ...
        for i in range(100):
            if random.random() < 0.5:
                ...
                if ik.solve(left_goal):
                    ...
                    Collision = self.planner.check_collision_free('left')
                    if Collision:
                ...
                if ik.solve(right_goal):
                    ...
                    Collision = self.planner.check_collision_free('right')
                    if Collision:
                        self.controller.setMilestone(self.robot.getConfig())
    ...
    def move_to_order_bin(self,object):
        ...
        for i in range(100):
            ...
            if ik.solve(goal,tol=0.1):
                ...
                Collision = self.planner.check_collision_free()
                if Collision:
                    self.controller.setMilestone(self.robot.getConfig())
                    return True
        return False
...
# Q1.
# There are several issues that can arise from running this code in a real-world setting. I will describe three of
# these and briefly describe how they might be addressed. These three problems consist of the following:
# 1. The first of these problems is the Baxter's arms colliding with the Baxter itself. For example, when changing
# from bin A to bin B, the robot switches arms but the right arm that moved to bin A did not move out of the way
# of the left arm that moved to bin B. This causes a collision between both the arms, as well as the head (monitor)
# of the Baxter. In order to overcome this issue, a collision-free trajectory needs to be planned such that both
# the Baxter kinematics and object in the configuration space are taken into account. This would involve implementing
# the collision avoidance bindings from the Klamp't directory.
# 2. A second issue that can be seen from running the initial simulation is the Baxter colliding with the bins.
# This is another obvious problem that must be solved before the code can be implemented on an actual Baxter robot.'
# To solve this issue we will have to implement the object avoidance library again, but this time use it for avoiding
# the bins. While this is not a concern for question 2 in this homework assignment, it will be a requirement for the
# actual Amazon Picking Challenge.
# 3. Another major issue with the initial simulation is the method in which the simulated Baxter grasps the objects.
# The initial simulation seems to grasp the tall object fairly easy (it seems that it grasps it a little tight although)
# as far as avoiding the bin shelves around it. The main problems arise when attempting to grasp the small and medium
# objects. With the initial simulation, the Baxter rotates the grippers to grasp the small and medium objects in an
# unconventional manner. By rotating the grippers 90 degrees clockwise it would be impossible to correctly pickup the
# small or medium objects without colliding with the base of the bin that those objects are in. Therefore, a new method
# of picking up these two types of objects needs to be implemented, such that the grippers do not rotate. Instead,
# they should just move into the bin, as they do for the tall objects, and align with either the small or medium
# objects. To do this we will design a new grasping method for when the perception module detects small or medium
# objects.
#
# Q2.
# My process is an iterative method that creates an ik objective and solves it until a collision free solution is found.
# The process of detecting a collision is done for all links and joints that could potentially collide with the body of
# the Baxter Robot.
#
# I ran 6 tests and recorded how many ik solutions were found to have collisions with the Baxters body. From these
# tests it was determined that 43% of the generated solutions were colliding with the body of the Robot.
#
# Q3.
#
#

...
class LimbPlanner:
    ...
    def check_collision_free(self,limb):
        ...
        id_to_index = dict([(self.robot.getLink(i).getID(),i) for i in range(self.robot.numLinks())])
        left_gripper_link_index = id_to_index[self.left_gripper_link.getID()]
        right_gripper_link_index = id_to_index[self.right_gripper_link.getID()]
        left_camera_link_index = id_to_index[self.left_camera_link.getID()]
        right_camera_link_index = id_to_index[self.right_camera_link.getID()]

        left_arm_hand = self.left_arm_indices
       # left_arm_hand.extend([left_gripper_link_index, left_camera_link_index])

        right_arm_hand = self.right_arm_indices
       # right_arm_hand.extend([right_gripper_link_index, right_camera_link_index])
         
     

        for i in left_arm_hand:
	    for j in right_arm_hand:
                if self.robot.getLink(i).geometry().collides(self.robot.getLink(j).geometry()):
                   return False                

     #   if limb == 'left':
     #      for i in left_arm_hand:
     #          for j in left_arm_hand:
     #              if i != j  and self.robot.getLink(i).geometry().collides(self.robot.getLink(j).geometry()):
     #                 return False     
   
     #   if limb == 'right':
     #      for i in right_arm_hand:
     #          for j in right_arm_hand:
     #              if i != j  and self.robot.getLink(i).geometry().collides(self.robot.getLink(j).geometry()):
     #                 return False     

...
class PickingController:
    ...
    def move_camera_to_bin(self,bin_name):
        ...
        for i in range(100):
            if random.random() < 0.5:
                ...
                if ik.solve(left_goal) and self.planner.check_collision_free('left'):
                    #TODO: plan a path
                ...
                if ik.solve(right_goal) and self.planner.check_collision_free('right'):
                    #TODO: plan a path
    ...
    def move_to_grasp_object(self,object):
        ...
        for (grasp,gxform) in grasps:
            ...
            self.robot.setConfig(qcmd)
            if ik.solve(goal) and self.planner.check_collision_free(self.active_limb):
                self.controller.setMilestone(self.robot.getConfig())
    ...
    def move_to_order_bin(self,object):
        ...
        for i in range(100):
            ...
            if i==0:
                ...
                self.randomize_limb_position(self.active_limb)
            #solve
            if ik.solve(goal,tol=0.1) and self.planner.check_collision_free(self.active_limb):
                #TODO: plan a path
                self.controller.setMilestone(self.robot.getConfig())
                return True
        return False
...
# Q1.The robot could collide with his other arm when trying to view a bin, or put the object in the order bin. It could 
#    also collide with the shelf/object. Even though the robot can view bin D with its right arm, it cannot grasp the 
#    object. The grasp is successful with the left arm. Perhaps if a grasp fails for an object, the other hand should 	
#    tried out automatically, to make sure that the object is grasped with the highest probability at first command.
#    
# Q2.I test for collisions between the links in the active arm and the other arm. They are tested right after ik.solve
#    in all of the methods. So, not only does ik.solve have to give a solution, the solution has to be such that the 
#    two arms do not collide. 
# Q3.
#
#
# Q4.

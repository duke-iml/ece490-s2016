...
t1 = so3.from_axis_angle(([1,0,0],math.pi*-0.5))
t2 = so3.from_axis_angle(([0,1,0],math.pi*-1.25))
t3 = so3.mul(t1,t2)
t4 = so3.from_axis_angle(([1,0,0],math.pi*0.025))
t3 = so3.mul(t3,t4)
x = ItemGrasp( (t3,[-0.015,0.02,-0.01] ))
x.pregrasp = (t3,[-.08,.08,-0.08])
small_item.grasps.append(x)


t1 = so3.from_axis_angle(([1,0,0],math.pi*0.35))
t2 = so3.from_axis_angle(([0,0,1],math.pi))
t3 = so3.mul(t1,t2)
x = ItemGrasp( (t3,[-0.015,0.02,-0.025] ))
x.pregrasp = (t3,[-0.015,0.1,-0.08])
small_item.grasps.append(x)
...
t1 = so3.from_axis_angle(([1,0,0],math.pi*-0.5))
t2 = so3.from_axis_angle(([0,1,0],math.pi*0.5))
t4 = so3.from_axis_angle(([1,0,0],math.pi*0.15))
t3 = so3.mul(so3.mul(t1,t2),t4)

x = ItemGrasp((t3,[-0.025,0.0,-0.008]))
x.pregrasp = (t3,[-0.13,0.0,-0.008])
med_item.grasps.append(x)

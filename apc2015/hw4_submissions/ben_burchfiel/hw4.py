...
# Q1. When picking up items, a few big issues reared their heads. Most obviously,
# the arm moved through objects (i.e.) no collision checking. Not only did it
# disregard the shelf, it also disregarded self collisions. More insidiously, many
# of the grip strategies seem to rely on the gripper actually moving through the shelf.
# These strategies require that half of the parallel gripper be under the object. Obviously
# this isn't actually possible, i.e. different grip strategies need to be employed.
#
# Another issue was the latency between where the baxter's arm was, and where the planning
# simulation indicated it was. The arm actually moves quite a bit slower than the planning simulation.
# This could cause issues if not taken into account. In the controller, one way to deal with this would be to
# insert delays after each motion (or motion segment) to give the physical arm time to synchronize with the planner.
# A more principled approach would be to close the loop, incorporate information from the baxter into the planner and
# attempt to correct the estimate, in real time, of where the physical arm is. Furthermore, not only was there severe
# delay, but the physical arm often took a slightly different route than the simulated arm. It is possible collisions
# might occur along the path taken by the physical arm even when the simulated arm is collision free. One approach to
# deal with this would be to require a collision free buffer around planned paths. Another, possibly complementary,
# option would be to break paths into smaller segments, possibly with pauses inserted between each segment. This might
# cause the physical arm to track the simulated one more closely.
#
# A third issue that seems like it will be a problem is conflict between the two arms. When switching between the two,
# the arm not being used is sometimes in the way of the active arm, potentially making a collision free path impossible
# to find. It seems like it will not be enough to simply check self collisions when planning; we may also need to
# actively move the arm that is not in use out of the way of the active arm.
#
# Q2. In the three functions that call the IK solver, I inserted a check directly after an IK solution was found.
# Where previously, the solver would retry (with random restarts on initial position) until a solution was found (or
# 100 iterations were exceed), now we also retry if an IK solution is found, but results in a self collision. To check
# for collisions, I used the (extremely awesome) WorldCollider class's robotSelfCollisions function. This tests for
# collisions against all robot link. I also added a counter to determine how many retries
# (after a successful IK solution was found) were needed to ensure no self collisions.
#
# To determine the prevalence of self collisions, random commands were issued to the robot (including grasp and move to
# order bin commands). After each command completed, the number of successful IK solutions that self collided was recorded.
# Surprisingly (for me at least) not many tries were generally needed to find self collision free solutions. Occasionally,
# somewhere on the order of 5 or 10 tries occurred however the vast majority of the time, only one or two repeats were
# required. One factor that seemed to help was that if the robot is in a non-self colliding state, small movements to
# other bins or forward often (though not always) resulted in a state that still did not self collide. It also seemed to
# help that the order bin was in an advantageous spot for the robot to not self collide when reaching towards it.
#
#
# Q3.
#

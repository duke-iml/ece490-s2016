...
public:
	RawPipeline(GeometryViewer* viewer=NULL,SocketPipeWorker* service=NULL)
		: projection(NULL), m_viewer(viewer), m_service(service), m_color_render(L"Color"),m_depth_render(L"Depth"), m_total_frames (0), m_next_transmit_time(0)
	{
	}
	~RawPipeline() {
		if(projection) {
